...
public class GardenTile : MonoBehaviour {
	...
	[SerializeField] private Color[ ] indicatedColors;
	...
	[SerializeField] private bool _isIndicated;
	...
	/// <summary>
	/// Whether or not this tile is being indicated that the placeable currently on it is being effected by another placeable
	/// </summary>
	...
	/// <summary>
	/// The info display that appears above the placeable on this tile when the player hovers the mouse over it
	/// </summary>
	...
	private void UpdateTile ( ) {
		...
		if (IsHighlighted) {
			...
			tempMaterial.color = ((_position.x + _position.y) % 2 == 0 ? indicatedColors[0] : indicatedColors[1]);
	...
	private void OnMouseEnter ( ) {
		...
		if (GardenPlaceable != null && playerDataManager.MouseSprite == null) {
			...
			foreach (GardenTile effectedGardenTile in GardenPlaceable.EffectedTiles) {
	...
	private void OnMouseExit ( ) {
		// If there is a garden placeable on this tile, then stop indicating what tiles it is effecting as the mouse has stopped hovering over this tile
		if (GardenPlaceable != null) {
			foreach (GardenTile effectedGardenTile in GardenPlaceable.EffectedTiles) {
